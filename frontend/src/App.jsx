...
export default function App() {
  ...
  const selectedModel =
    MODEL_REGISTRY.find((m) => m.id === selectedModelId) ?? null;
  const {
    isLoading: modelLoading,
    error: modelError,
    runInference,
  } = useYOLO(selectedModelId !== "none" ? selectedModel : null);
  ...
  // ── Telemetry snapshot for frame tagging ──────────────────────────────────
  const telemetry = useMemo(
    () => ({
      roll: attitude?.roll ?? 0,
      pitch: attitude?.pitch ?? 0,
      yaw: attitude?.yaw ?? 0,
      depth: pressure?.depth ?? 0,
      batteryPct: batteryPct ?? 0,
      armed,
      currentMode,
    }),
    [attitude, pressure, batteryPct, armed, currentMode],
  );

  // ── Inspection / gallery state ────────────────────────────────────────────
  const [tags,        setTags]        = useState([]);
  const [activeTab,   setActiveTab]   = useState("feed");

  const taggedCount = tags.length;

  // Restore tags from previous session on mount
  useEffect(() => {
    fetch("/api/tags")
      .then((r) => r.json())
    ...
    async (dataUrl) => {
      try {
        const res = await fetch("/api/tags", {
          ...
          body: JSON.stringify({ frame: dataUrl, telemetry, detections }),
        });
  ...
  const handleDownloadReport = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = `rov-report-${Date.now()}.html`;
      a.click();
  ...
  const handleResetTags = useCallback(async () => {
    if (!window.confirm("Delete all tagged frames?")) return;
    try {
      const res = await fetch("/api/tags", { method: "DELETE" });
      if (res.ok) {
        setTags([]);
        if (activeTab === "gallery") setActiveTab("feed");
      }
    } catch (e) {
  ...
  const isManualMode = useCallback(() => {
    const mode = Number.isFinite(currentMode)
      ? currentMode
      : parseInt(currentMode, 10);
  ...
  const handleModeChange = useCallback(
    (modeInput) => {
      ...
      if (!manualModeSet.has(mode)) {
        ...
        sendManualControlState(neutral, {
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      const source =
        videoMode === "mjpeg" ? mjpegRef.current : videoRef.current;
      const ready =
        source &&
  ...
  const onAxisPress = useCallback(
    (axis, value) => {
      ...
      sendManualControlState(
        { ...manualStateRef.current },
        { bypassModeCheck: true, immediate: true },
      );
    },
    [sendManualControlState],
  );
